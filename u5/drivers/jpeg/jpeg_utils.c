/**
  ...
  * @brief   This driver provides JPEG MCU (Minimum Coded Unit) blocks to RGB
  and RGB to JPEG MCU conversion functions.
  ******************************************************************************
  * @attention
  *
  ...
  ******************************************************************************
  @verbatim
    [..]
      The STM32 HW JPEG decoder/encoder peripheral allows to decode/encode jpeg
  images. In decoding, the STM32 HW JPEG data output are organized in blocks
  called MCU (Minimum Coded Unit) as specified in the jpeg standard. In
  encoding, the STM32 HW JPEG data input must be organized in MCU blocks.
      Depending of the image color space and chrominance sampling, an MCU is
  generally organized in : N luminance (Y) blocks + a Blue chrominance (Cb)
  block + a Red chrominance (Cr) block. Each block size is 8x8 samples.
      ...
      The STM32 HW JPEG supports 3 possible color space :
        ...
        - CMYK : an MCU is composed of 4 color components : Cyan, Magenta,
  Yellow, and Key (blacK)

      This utility file should be used on top of the STM32 HAL JPEG driver.
      In decoding, it allows to convert MCU blocks to RGB888 or ARGB8888 pixels
  stored to a destination frame buffer that can be displayed. In encoding, it
  allows to convert RGB888 or ARGB8888 pixels  to MCU blocks.

      This utility supports following MCU Chroma sampling format and color space
  :
        - YCbCr 4:2:0 : Each MCU is composed of 4 Y 8x8 blocks + 1 Cb 8x8 block
  + Cr 8x8 block
        - YCbCr 4:2:2 : Each MCU is composed of 2 Y 8x8 blocks + 1 Cb 8x8 block
  + Cr 8x8 block
        - YCbCr 4:4:4 : Each MCU is composed of 1 Y 8x8 block + 1 Cb 8x8 block +
  Cr 8x8 block
        - GrayScale   : Each MCU is composed of 1 Y 8x8 block
        - CMYK        : Each MCU is composed of 1 Cyan 8x8  block + 1 Magenta
  8x8 block + 1 Yellow 8x8 block + 1 Key 8x8 block.
  ==============================================================================
                     ##### How to use this driver #####
  ==============================================================================
    [..]
      - The configuration file "jpeg_utils_conf_template.h" is used to configure
  this utility providing some useful flexibilities. This file should be copied
  to the application folder and modified as follows:
          - Rename it to "jpeg_utils_conf.h".
          - Update the name of the JPEG driver's header file, depending on the
  EVAL board you are using.

      Example if using the STM32F769I-EVAL board :
        - Copy the file "jpeg_utils_conf_template.h" to the application folder
  and rename it to "jpeg_utils_conf.h"
      ...
      Using this configuration file, user can change the following settings:
          - Use RGB888 or ARGB8888 or RGB565 by setting the constant
  JPEG_RGB_FORMAT respectively to JPEG_RGB888, JPEG_ARGB8888 JPEG_RGB565.
          - Swap RED, and Blue offsets if user needs to change the color order
  to BGR (instead of RGB) by setting: #define JPEG_SWAP_RB     1
          - Enable or disable the decoding post-processing functions (YCbCr to
  RGB conversion functions) by setting the define USE_JPEG_DECODER respectively
  to 0 or 1.
          - Enable or disable the encoding pre-processing functions (RGB to
  YCbCr conversion functions) by setting the define USE_JPEG_ENCODER
            respectively to 0 or 1.

    [..]For Decoding:
      - First, function "JPEG_InitColorTables" should be called to initialize
  the YCbCr to RGB color conversion tables. This function needs to be called
  only one time at the beginning of the program whatever the number of jpeg
  files to be decoded.

      - As soon as the JPEG HW peripheral finished parsing the header of the
  JPEG input file, the HAL JPEG callback "HAL_JPEG_InfoReadyCallback" is
  launched providing the jpeg file characteristics found in its header. User can
  then call the utility function "JPEG_GetDecodeColorConvertFunc" with these
        information and retrieve the corresponding color conversion function and
  number of MCUs.

        Then each time an integer number of MCUs are available (from the HW JPEG
  output), user can call the retrieved function to convert these HW JPEG output
  data to RGB888 or ARGB8888 pixel stored to the specified destination buffer.

    [..]For Encoding:
      - First, function "JPEG_InitColorTables" should be called to initialize
  the YCbCr/RGB color conversion tables. This function needs to be called only
  one time at the beginning of the program whatever the number of jpeg files to
  be encoded or decoded.

      - First Use the utility function "JPEG_GetEncodeColorConvertFunc" with the
  input image information to retrieve the corresponding color conversion
  function and number of MCUs.

        Then each time an RGB input buffer is available, user can call the
  retrieved function to convert RGB data to MCU blocks stored to the specified
  destination buffer.
  @endverbatim
  */
...
#define JPEG_BYTES_PER_PIXEL                                                   \
  2 /* Number of bytes in a pixel                   */
#define JPEG_RGB565_GREEN_MASK                                                 \
  0x07E0 /* Mask of Green component in RGB565 Format     */

#if (JPEG_SWAP_RB == 0)
#define JPEG_RED_OFFSET 11 /* Offset of the RED color in a pixel           */
#define JPEG_BLUE_OFFSET 0 /* Offset of the BLUE color in a pixel          */
#define JPEG_RGB565_RED_MASK                                                   \
  0xF800 /* Mask of Red component in RGB565 Format       */
#define JPEG_RGB565_BLUE_MASK                                                  \
  0x001F /* Mask of Blue component in RGB565 Format      */
#else
#define JPEG_RED_OFFSET 0   /* Offset of the RED color in a pixel           */
#define JPEG_BLUE_OFFSET 11 /* Offset of the BLUE color in a pixel          */
#define JPEG_RGB565_RED_MASK                                                   \
  0x001F /* Mask of Red component in RGB565 Format       */
#define JPEG_RGB565_BLUE_MASK                                                  \
...
static int32_t
    CR_GREEN_LUT[256]; /* Cr to Green color conversion Look Up Table*/
static int32_t
    CB_GREEN_LUT[256]; /* Cb to Green color conversion Look Up Table*/
...
static int32_t BLUE_CB_RED_CR_LUT[256]; /* Red to Cr and Blue to Cb color
                                           conversion Look Up Table  */
static int32_t GREEN_Y_LUT[256]; /* Green to Y color conversion Look Up Table*/
static int32_t
    GREEN_CR_LUT[256]; /* Green to Cr color conversion Look Up Table*/
static int32_t
    GREEN_CB_LUT[256];           /* Green to Cb color conversion Look Up Table*/
...
#if (USE_JPEG_ENCODER == 1)
static uint32_t
JPEG_ARGB_MCU_YCbCr420_ConvertBlocks(uint8_t *pInBuffer, uint8_t *pOutBuffer,
                                     uint32_t BlockIndex, uint32_t DataCount,
                                     uint32_t *ConvertedDataCount);

static uint32_t
JPEG_ARGB_MCU_YCbCr422_ConvertBlocks(uint8_t *pInBuffer, uint8_t *pOutBuffer,
                                     uint32_t BlockIndex, uint32_t DataCount,
                                     uint32_t *ConvertedDataCount);

static uint32_t
JPEG_ARGB_MCU_YCbCr444_ConvertBlocks(uint8_t *pInBuffer, uint8_t *pOutBuffer,
                                     uint32_t BlockIndex, uint32_t DataCount,
                                     uint32_t *ConvertedDataCount);
...
#if (USE_JPEG_DECODER == 1)
static uint32_t
JPEG_MCU_YCbCr420_ARGB_ConvertBlocks(uint8_t *pInBuffer, uint8_t *pOutBuffer,
                                     uint32_t BlockIndex, uint32_t DataCount,
                                     uint32_t *ConvertedDataCount);

static uint32_t
JPEG_MCU_YCbCr422_ARGB_ConvertBlocks(uint8_t *pInBuffer, uint8_t *pOutBuffer,
                                     uint32_t BlockIndex, uint32_t DataCount,
                                     uint32_t *ConvertedDataCount);

static uint32_t
JPEG_MCU_YCbCr444_ARGB_ConvertBlocks(uint8_t *pInBuffer, uint8_t *pOutBuffer,
                                     uint32_t BlockIndex, uint32_t DataCount,
                                     uint32_t *ConvertedDataCount);
 ...
 * @param  BlockIndex : index of the input buffer first block in the final
 * image.
...
JPEG_ARGB_MCU_YCbCr420_ConvertBlocks(uint8_t *pInBuffer, uint8_t *pOutBuffer,
  ...
  numberMCU =
      ((3 * DataCount) / (2 * JPEG_BYTES_PER_PIXEL * YCBCR_420_BLOCK_SIZE));
  ...
  *ConvertedDataCount = numberMCU * JPEG_ConvertorParams.BlockSize;
  ...
  while (currentMCU < (numberMCU + BlockIndex)) {
    xRef = ((currentMCU * JPEG_ConvertorParams.H_factor) /
            JPEG_ConvertorParams.WidthExtend) *
           JPEG_ConvertorParams.V_factor;

    yRef = ((currentMCU * JPEG_ConvertorParams.H_factor) %
            JPEG_ConvertorParams.WidthExtend);

    refline =
        JPEG_ConvertorParams.ScaledWidth * xRef + (JPEG_BYTES_PER_PIXEL * yRef);

    currentMCU++;

    if (((currentMCU * JPEG_ConvertorParams.H_factor) %
         JPEG_ConvertorParams.WidthExtend) == 0) {
    ...
    for (i = 0; i < JPEG_ConvertorParams.V_factor; i += 2) {
      ...
      for (j = 0; j < colones; j += 2) {
        ...
        red =
            (((*(__IO uint16_t *)(pInAddr + refline)) & JPEG_RGB565_RED_MASK) >>
             JPEG_RED_OFFSET);
        green = (((*(__IO uint16_t *)(pInAddr + refline)) &
                  JPEG_RGB565_GREEN_MASK) >>
                 JPEG_GREEN_OFFSET);
        blue = (((*(__IO uint16_t *)(pInAddr + refline)) &
...
#endif
        ycomp = (int32_t)(*(RED_Y_LUT + red)) +
                (int32_t)(*(GREEN_Y_LUT + green)) +
                (int32_t)(*(BLUE_Y_LUT + blue));
        cbcomp = (int32_t)(*(RED_CB_LUT + red)) +
                 (int32_t)(*(GREEN_CB_LUT + green)) +
                 (int32_t)(*(BLUE_CB_RED_CR_LUT + blue)) + 128;
        crcomp = (int32_t)(*(BLUE_CB_RED_CR_LUT + red)) +
                 (int32_t)(*(GREEN_CR_LUT + green)) +
        ...
        red = (((*(__IO uint16_t *)(pInAddr + JPEG_BYTES_PER_PIXEL + refline)) &
               ...
               JPEG_RED_OFFSET);
        green =
            (((*(__IO uint16_t *)(pInAddr + JPEG_BYTES_PER_PIXEL + refline)) &
              JPEG_RGB565_GREEN_MASK) >>
             JPEG_GREEN_OFFSET);
        blue =
            (((*(__IO uint16_t *)(pInAddr + JPEG_BYTES_PER_PIXEL + refline)) &
...
#else
        red =
            (*(pInAddr + refline + JPEG_BYTES_PER_PIXEL + JPEG_RED_OFFSET / 8));
        green = (*(pInAddr + refline + JPEG_BYTES_PER_PIXEL +
                   JPEG_GREEN_OFFSET / 8));
        blue = (*(pInAddr + refline + JPEG_BYTES_PER_PIXEL +
                  JPEG_BLUE_OFFSET / 8));
#endif
        ycomp = (int32_t)(*(RED_Y_LUT + red)) +
                (int32_t)(*(GREEN_Y_LUT + green)) +
        ...
        /* Third Pixel */
#if (JPEG_RGB_FORMAT == JPEG_RGB565)
        red = (((*(__IO uint16_t *)(pInAddr + JPEG_ConvertorParams.ScaledWidth +
                                    refline)) &
                JPEG_RGB565_RED_MASK) >>
               JPEG_RED_OFFSET);
        green =
            (((*(__IO uint16_t *)(pInAddr + JPEG_ConvertorParams.ScaledWidth +
                                  refline)) &
              JPEG_RGB565_GREEN_MASK) >>
             JPEG_GREEN_OFFSET);
        blue =
        ...
        (*(pOutAddr + JPEG_ConvertorParams
                          .Y_MCU_LUT[offset + JPEG_ConvertorParams.H_factor])) =
            (ycomp);

        /* Fourth Pixel */
#if (JPEG_RGB_FORMAT == JPEG_RGB565)
        red = (((*(__IO uint16_t *)(pInAddr + refline +
                                    JPEG_ConvertorParams.ScaledWidth +
        ...
        (*(pOutAddr +
           JPEG_ConvertorParams
               .Y_MCU_LUT[offset + JPEG_ConvertorParams.H_factor + 1])) =
            (ycomp);
      ...
      offset += JPEG_ConvertorParams.H_factor +
                (JPEG_ConvertorParams.H_factor - colones);
    ...
    for (i = 0; i < JPEG_ConvertorParams.V_factor; i += 1) {
      ...
      for (j = 0; j < colones; j += 2) {
        ...
        red = (((*(__IO uint16_t *)(pInAddr + refline + JPEG_BYTES_PER_PIXEL)) &
               ...
               JPEG_RED_OFFSET);
        green =
            (((*(__IO uint16_t *)(pInAddr + refline + JPEG_BYTES_PER_PIXEL)) &
              JPEG_RGB565_GREEN_MASK) >>
             JPEG_GREEN_OFFSET);
        blue =
            (((*(__IO uint16_t *)(pInAddr + refline + JPEG_BYTES_PER_PIXEL)) &
  ...
  while (currentMCU < (numberMCU + BlockIndex)) {
    ...
    for (i = 0; i < JPEG_ConvertorParams.V_factor; i++) {
      ...
      for (j = 0; j < colones; j++) {
        ...
        blue = (*(pInAddr + refline + JPEG_BLUE_OFFSET / 8));
#endif
        ycomp = (uint8_t)((int32_t)(*(RED_Y_LUT + red)) +
                          (int32_t)(*(GREEN_Y_LUT + green)) +
                          (int32_t)(*(BLUE_Y_LUT + blue)));
  ...
  while (currentMCU < (numberMCU + BlockIndex)) {
    ...
    for (i = 0; i < JPEG_ConvertorParams.V_factor; i++) {
      ...
      for (j = 0; j < colones; j++) {
        ...
        (*(pOutAddr + JPEG_ConvertorParams.Y_MCU_LUT[offset])) =
            red * 255 / color_k;
        (*(pOutAddr + JPEG_ConvertorParams.Cb_MCU_LUT[offset])) =
            green * 255 / color_k;
        (*(pOutAddr + JPEG_ConvertorParams.Cr_MCU_LUT[offset])) =
            blue * 255 / color_k;
    ...
    JPEG_Set_K_Blocks(pOutAddr, kBlocks,
                      JPEG_ConvertorParams.ChromaSubsampling);
    pOutAddr += JPEG_ConvertorParams.BlockSize;
  }
...
/**
 * @brief  Retrieve Encoding RGB to YCbCr color conversion function and block
 * number
 * @param  pJpegInfo  : JPEG_ConfTypeDef that contains the JPEG image
 * information. These info are available in the HAL callback
 * "HAL_JPEG_InfoReadyCallback".
 * @param  pFunction  : pointer to JPEG_RGBToYCbCr_Convert_Function , used to
 * Retrieve the color conversion function depending of the jpeg image color
 * space and chroma sampling info.
 * @param ImageNbMCUs : pointer to uint32_t, used to Retrieve the total number
 * of MCU blocks in the jpeg image.
...
JPEG_GetEncodeColorConvertFunc(JPEG_ConfTypeDef *pJpegInfo,
  ...
  JPEG_ConvertorParams.ScaledWidth =
      JPEG_BYTES_PER_PIXEL * JPEG_ConvertorParams.ImageWidth;
...
/**
 * @brief  Retrieve Decoding YCbCr to RGB color conversion function and block
 * number
 * @param  pJpegInfo  : JPEG_ConfTypeDef that contains the JPEG image
 * information. These info are available in the HAL callback
 * "HAL_JPEG_InfoReadyCallback".
 * @param  pFunction  : pointer to JPEG_YCbCrToRGB_Convert_Function , used to
 * Retrieve the color conversion function depending of the jpeg image color
 * space and chroma sampling info.
 * @param ImageNbMCUs : pointer to uint32_t, used to Retrieve the total number
 * of MCU blocks in the jpeg image.
...
void JPEG_InitPreProcColorTables(void) {
  ...
  for (i = 0; i <= 255; i++) {
    RED_Y_LUT[i] =
        ((((int32_t)((0.299) * (1L << 16))) * i) + ((int32_t)1 << (16 - 1))) >>
        16;
    GREEN_Y_LUT[i] =
        ((((int32_t)((0.587) * (1L << 16))) * i) + ((int32_t)1 << (16 - 1))) >>
        16;
    BLUE_Y_LUT[i] =
        ((((int32_t)((0.114) * (1L << 16))) * i) + ((int32_t)1 << (16 - 1))) >>
        16;

    RED_CB_LUT[i] = (((-((int32_t)((0.1687) * (1L << 16)))) * i) +
                     ((int32_t)1 << (16 - 1))) >>
                    16;
    GREEN_CB_LUT[i] = (((-((int32_t)((0.3313) * (1L << 16)))) * i) +
                       ((int32_t)1 << (16 - 1))) >>
                      16;

    /* BLUE_CB_LUT and RED_CR_LUT are identical */
    BLUE_CB_RED_CR_LUT[i] =
        ((((int32_t)((0.5) * (1L << 16))) * i) + ((int32_t)1 << (16 - 1))) >>
        16;

    GREEN_CR_LUT[i] = (((-((int32_t)((0.4187) * (1L << 16)))) * i) +
                       ((int32_t)1 << (16 - 1))) >>
                      16;
    BLUE_CR_LUT[i] = (((-((int32_t)((0.0813) * (1L << 16)))) * i) +
                      ((int32_t)1 << (16 - 1))) >>
                     16;
  }
}
