...
extern "C" const uint8_t *image_data(void) { return _binary_image_raw_start; }
...
extern "C" void edge_detect(const uint8_t *rgb_in, uint8_t *edges_out,
                            int width, int height, double low_thresh = 50.0,
...
extern "C" int edge_demo(const char *path, uint8_t *edges_out, int *width,
                         int *height, double low_thresh = 50.0,
                         double high_thresh = 150.0) {
  ...
  cv::cvtColor(
      src, gray,
      cv::COLOR_BGR2GRAY); // BGR if desktop; RGB if you built differently
...
extern "C" int object_detect(const char *img_path, const char *cascade_path) {
  ...
  // Optional: light histogram equalization (cheap, but you can skip if
  // CPU-tight)
  ...
  const int flags =
      0; // cv::CASCADE_FIND_BIGGEST_OBJECT | cv::CASCADE_DO_ROUGH_SEARCH;

  std::vector<cv::Rect> faces;
  faceCascade.detectMultiScale(gray, faces, scaleFactor, minNeighbors, flags,
                               minSize, maxSize);

  // If you only need a yes/no, you could return (faces.empty() ? 0 : 1) to
  // avoid vector copies
...
extern "C" int
object_detect_2(const char *img_path, const char *cascade_path,
                uint8_t **out_buf, // RGB888 buffer you can feed LVGL
                int *out_w, int *out_h,
                size_t *out_data_sz) // bytes in buffer
{
  if (!out_buf || !out_w || !out_h || !out_data_sz)
    return -99;
