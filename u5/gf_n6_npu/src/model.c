...
#define AI_OD_YOLOV2_PP_NB_INPUT_BOXES                                         \
  (AI_OD_YOLOV2_PP_GRID_WIDTH * AI_OD_YOLOV2_PP_GRID_HEIGHT)
static const float32_t AI_OD_YOLOV2_PP_ANCHORS[2 * AI_OD_YOLOV2_PP_NB_ANCHORS] =
    {
        0.9883000000f, 3.3606000000f,  2.1194000000f, 5.3759000000f,
        3.0520000000f, 9.1336000000f,  5.5517000000f, 9.3066000000f,
...
static uint8_t pScratchBuffer[(
    AI_OD_YOLOV2_PP_GRID_WIDTH * AI_OD_YOLOV2_PP_GRID_HEIGHT *
    AI_OD_YOLOV2_PP_NB_ANCHORS * sizeof(od_pp_outBuffer_t))] = {0};
...
void model_thread_ep(void *arg1, void *arg2, void *arg3) {
  ...
  pp_params.pScratchBuffer = pScratchBuffer;
  ...
  while (1) {
    ...
    if (ret) {
      LOG_ERR("Failed: sys_cache_data_invd_range %d", ret);
    }
    __ASSERT_NO_MSG(ret == 0);
    /* let's go */
    tick = HAL_GetTick();
